...
│   └── lib/
│       ├── browser-runner.mjs
│       ├── sources/{yandex,gdebenz,twogis,benzonavt}.mjs
│       ├── normalize.mjs
│       ├── fuels.mjs
│       ├── geometry.mjs
│       ├── identity.mjs
...
5. For coordinate-bearing stations returned by the nearby view, read the page's same-origin `/api/comments/{stationId}/recent?limit=12` timeline with bounded concurrency and a short per-request timeout. Preserve structured petrol-grade reports and explicit negative-to-positive transitions; exclude diesel and never parse queue counts such as `100+ машин` as an octane grade. A negative event changes a petrol grade only when that grade is named, or all known petrol grades when the source text explicitly says that all fuel/petrol is unavailable; a diesel-only negative cannot seed a petrol transition.
6. Preserve unknown raw fields as provenance. Do not assume an undocumented freshness-band order until fixtures or cross-source observations validate it.
...
Grade-blind positive or negative reports never become direct evidence for a raw source variant. `CURRENT_FAMILY` downgrades activity as well as observations unless the adapter separately declares that a structured history record explicitly owns its petrol grade. Raw variant scope is retained for provenance and conservative negative coverage, but every AI-95 variant contributes to one user-facing octane-grade assessment.
...
6. When the page exposes `benzin.api.2gis.ru` station data, retain current petrol-grade statuses from `by-ids` and bounded detail timelines from `/stations/{id}`. Detail calls use bounded concurrency and a short individual timeout; a failure preserves already collected current rows and marks activity-history coverage partial. Grade-specific UGC reports may establish a source-reported status transition. Transactions whose `fuel_types` are absent remain station-level context only: diesel cannot be excluded, so they never prove petrol availability or train the petrol forecast.
...
5. Compare `st.conflict.created_at` with `st.updated_at`. A valid conflict strictly older than the current status is superseded and does not suppress that status. A newer, equal-time, missing-time, or malformed conflict remains `UNCERTAIN`, with both raw sides preserved; clock ambiguity therefore still fails closed.
6. Normalize live queue shape `{at,size,until}`: `20_50 → LONG`, `gt50 → VERY_LONG`, and use `queue.at` rather than the fuel timestamp.
7. Read bounded detail data from same-origin `/api/v1/stations/{id}` only for coordinate-bearing rows, with bounded concurrency and a short individual timeout. Preserve `recent` petrol-grade reports and explicit status transitions; retain day-level `status_history` only as coarse provenance because it has no transition time. A failed optional detail request never discards the station's current status; it reduces the persisted activity-history coverage and makes source health `PARTIAL/ACTIVITY_HISTORY_PARTIAL`. Static price history remains irrelevant to availability.
8. Keep Benzonavt in the same default `crowd-shared` provenance group as Yandex/gdebenz so correlated crowd data cannot manufacture independent-source confidence.

All four adapters are enabled by default in order `yandex`, `gdebenz`, `2gis`, `benzonavt`, so the strongest expected evidence is collected first under the shared deadline. Optional per-station detail work shares a ten-second source-local budget in addition to individual request timeouts, so many stalled detail calls cannot consume the entire adapter deadline. A source can be disabled in adjacent config. Before release, live feasibility records every required first-party/resource domain for each adapter; a missing allowlisted CDN/resource is `RESOURCE_BLOCKED`, not `SCHEMA_CHANGED`.
...
An explicit source timeline may also yield `SOURCE_REPORTED_TRANSITION` when a readable petrol grade follows a readable negative for that grade or an explicit all-petrol negative. It is labelled with the source terminology and remains weaker than a verified payment-transaction resumption. Reports may quote the earliest source transition in the latest corroborated episode only while the station is currently positive, the transition is still inside the configured current-evidence retention, and it belongs to AI-95; later services are shown as lagging confirmations rather than separate deliveries. Grade-specific resumed activity is likewise current evidence only while fresh. A newer direct negative wins after the conflict window; a near-simultaneous negative and resumption are `CONFLICTING`.
...
Stations without positive current evidence are excluded from the primary recommendation list. Activity can affect the AI-95 row and its rank only when it belongs to octane 95 and its latest event remains inside the configured freshness window; fresh AI-92 activity and stale AI-95 activity have zero rank weight. Rank positive stations lexicographically:
...
Outside the temporary monitor directory, `collect.mjs` maintains a compact seven-day history in the user's state directory (or an explicit `--history` path) for both on-demand and monitoring runs. It stores only timestamp, scope hashes, station metadata, every explicit `source:sourceStationId` member, union/per-product verdicts, grade labels, rolling window/count/latest-event summaries, and compact source-reported transition timestamps. Raw pages, raw event lists, HARs, and full observations are not retained. Per-source activity-history coverage is persisted in the current snapshot's coverage metrics rather than copied into every history tick. History continuity is resolved by transitive member-ID overlap, so `merged:A+B → source:B → merged:A+B` remains one physical station even without `--previous`. Both tick `fetchedAt` and the source event's own `observedAt`/`resumedAt` must fall inside retention: a detail API repeatedly returning an old event cannot keep it alive through fresh ticks. Entries older than seven days are removed on every successful collection, and duplicate retries for the same `fetchedAt`/scope replace rather than duplicate a tick.
...
Forecast training uses general petrol-delivery candidates from gasoline grades 92/95/98/100. Diesel is explicitly excluded because its near-continuous availability would bias the sought petrol-restocking pattern; LPG, unrecognized grades, and other octane numbers are excluded as well. Before consecutive ticks are compared, raw variants are aggregated by station, source, tick, and octane grade. Rolling counts are summed across variants, `windowMinutes` uses the conservative minimum, and the latest valid grade-owned event timestamp is retained. Status union uses deterministic precedence `IN_STOCK → LIMITED → OUT_OF_STOCK → UNKNOWN`: an unreadable variant does not suppress another readable status, while a wholly unreadable octane remains unknown. Every octane-level event additionally requires a same-variant witness present in both adjacent ticks: readable `OUT_OF_STOCK → IN_STOCK/LIMITED` for statuses or `count 0 → count > 0` for rolling activity. Recognized variants use stable `productKey` identity; the shared `AI95_UNKNOWN` placeholder is never an identity and falls back to the normalized raw label so distinct unknown names cannot witness each other. A row merely appearing, disappearing, or becoming readable cannot create an event. A true per-grade rolling-summary event has highest priority and requires the same source/station/octane/window in consecutive samples, a previous zero count whose window spans the configured quiet gap, and a current count of at least two with a recent latest-event timestamp. A station-level aggregate count must never be copied onto grades or used to raise a grade's confidence: live Yandex evidence on 2026-08-30 showed `signalsCountPerHour` and `lastSignalTimestamp` only on the `fuelAvailability` container, while each 92/95/98/100/DIESEL row contained only `fuelType`, `localizedName`, and `status`; the aggregate is therefore ineligible because diesel cannot be removed from it. Browser-mediated detail timelines from gdebenz, 2GIS, and Benzonavt add explicit source-reported petrol transitions. Reports from different services within one 120-minute station window are clustered as one physical replenishment episode and use the earliest report time; they never inflate the sample count merely by corroborating the same delivery. A typical-time pattern additionally requires qualifying episodes on at least two distinct Moscow-local dates. Observed union `NOT_AVAILABLE → AVAILABLE` transitions provide a weaker outage-duration fallback. All event types remain tanker-arrival heuristics and do not prove the delivered product mix. The forecast estimates typical Moscow-local time of day from at least two events, in order of same station, normalized brand, then area. Gaps longer than three tick intervals break transition continuity. The report gives a point estimate, interquartile window, signal type, scope, sample size, and confidence, and emits no invented time without enough history.

The tanker model is intentionally cross-octane for petrol: a witnessed return of AI-92, AI-95, AI-98, or AI-100 may train the station's generic delivery clock and therefore an AI-95 appearance forecast, because one tanker normally replenishes several petrol grades. Diesel remains excluded. This inference affects forecasts only; a current AI-92 signal alone never changes the current AI-95 verdict or makes the station eligible for `Куда ехать`.

The two-date requirement applies to time-of-day models (`ROLLING_ACTIVITY`, source timelines, and petrol-status clock patterns). The outage-duration fallback is not a clock-time model: it may learn from two completed outages on the same Moscow-local date because it predicts elapsed minutes from the current negative start.
