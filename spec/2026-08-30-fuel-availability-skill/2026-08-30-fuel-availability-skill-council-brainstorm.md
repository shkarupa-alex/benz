...
5. For stations returned by the nearby view, read the page's same-origin `/api/comments/{stationId}/recent?limit=12` timeline with bounded concurrency. Preserve structured petrol-grade reports and explicit negative-to-positive transitions; exclude diesel and never parse queue counts such as `100+ машин` as an octane grade.
...
7. Read bounded detail data from same-origin `/api/v1/stations/{id}`. Preserve `recent` petrol-grade reports and explicit status transitions; retain day-level `status_history` only as coarse provenance because it has no transition time. Static price history remains irrelevant to availability.
...
An explicit source timeline may also yield `SOURCE_REPORTED_TRANSITION` when a readable petrol grade follows a readable source negative. It is labelled with the source terminology and remains weaker than a verified payment-transaction resumption. Reports for a currently positive station may quote the earliest source transition in the latest corroborated episode; later services are shown as lagging confirmations rather than separate deliveries.
