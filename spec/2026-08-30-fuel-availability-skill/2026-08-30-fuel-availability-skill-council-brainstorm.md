...
The global history read–modify–write is serialized by a sibling exclusive lock file with a five-second acquisition bound. A dead PID owner is recovered under a separate exclusive reclaim lock; a malformed owner record becomes recoverable only after 30 seconds. Both release paths verify an owner nonce before unlinking, so an old owner cannot remove its successor's lock. The lock covers read, pruning, append, forecast calculation, and atomic rename and is released in `finally`. Concurrent collectors therefore retain every tick. A lock timeout surfaces as `HISTORY_UNAVAILABLE`/`HISTORY_LOCK_TIMEOUT` without suppressing the current collection report.
