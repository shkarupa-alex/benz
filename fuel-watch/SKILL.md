...
Все сайты открываются только через `agent-browser`, которым управляют скрипты. Не добавляй `curl`, `fetch`, прямой HTTP-клиент, CAPTCHA solver, dashboard, HAR, video, trace, restore/profile или фоновый браузер.
...
Если пользователь просит одновременно показать все источники, используй отдельный **visual diagnostic mode**: одно окно, один namespace и одна session, `AGENT_BROWSER_IDLE_TIMEOUT_MS=3600000`, только точные URL источников из конфига. Не передавай `--allowed-domains` в этой видимой session: agent-browser 0.35.1 при применении CDP network controls к третьей вкладке воспроизводимо теряет execution context и сбрасывает предыдущие tabs. Вместо этого после каждой навигации прочитай final URL и fail-closed проверь host по `config.browser.allowedDomains`; при внешнем redirect сразу закрой owned session и сообщи `RESOURCE_BLOCKED`. Это исключение относится только к видимому ручному осмотру; автоматический collect всегда сохраняет network controls.

Первую страницу Yandex открой через `open --headed` (она останется с системным label вроде `t1`), затем сразу создай только две вкладки через `tab new --label gdebenz` и `tab new --label two-gis`. Label всегда начинай с буквы. Не создавай временную четвёртую вкладку ради переименования первой и не создавай отдельную session для каждого источника: отдельные sessions запускают отдельные окна. После каждого `tab new` проверяй `tab list` и `reused`; при `launched: true`, потере предыдущих вкладок или `about:blank` сообщи `BROWSER_UNAVAILABLE`, а не считай источники открытыми. На agent-browser 0.35.1 не запускай `screenshot` в этой трёхвкладочной session: наблюдалось зависание command queue; для диагностики используй короткие `get`, `snapshot` и `eval`. Видимая diagnostic session не является collection-run и закрывается только по команде пользователя либо после согласованного осмотра.
