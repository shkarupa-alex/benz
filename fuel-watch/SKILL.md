...
Штатная collection всегда использует `HEADED` Chromium без stealth-флагов и подмены fingerprint. Единственное исключение — User-Agent: если браузер фактически поднялся headless (на сервере без display это происходит несмотря на `--headed`) и источник ответил на это своей страницей автолимита, выполняется одна повторная попытка с той же личностью браузера, из которой убран токен `Headless`. Версия и платформа остаются настоящими, другой браузер не изображается, CAPTCHA не обходится; при желании строку можно задать явно через `browser.userAgent`. Если доступен пользовательский display, BrowserRunner сохраняет `DISPLAY`/`WAYLAND_DISPLAY`/`XAUTHORITY`; иначе `agent-browser` может использовать собственный Xvfb, но браузер всё равно остаётся non-headless. Не переноси результат `HEADLESS`-диагностики на обычный браузер или доступность сайта вообще. Каждый источник получает отдельную короткоживущую session, источники обрабатываются строго последовательно, поэтому одновременно активно не более одного окна.

BrowserRunner сначала обязательно устанавливает `--allowed-domains`. Только если agent-browser (проверено на 0.38.1) возвращает известную CDP-ошибку установки network controls, он один раз пересоздаёт owned session без controls и сохраняет fail-closed проверки точного final host и последующего page drift. Такой результат всегда получает предупреждение `BROWSER_NETWORK_CONTROLS_DEGRADED`; не скрывай его и не называй fallback эквивалентом полноценной сетевой изоляции.
...
- Ассортимент АЗС (`assortment`, `sellsRequestedFamily`) отвечает на другой вопрос, чем наличие: «здесь вообще не продают 95» — это не дефицит. Такие станции считаются отдельной строкой «не продают АИ-95» и не получают прогноз подвоза. Ассортимент публикуют только 2GIS, Benzonavt и часть строк gdebenz; отсутствие каталога означает «неизвестно», а не «не продают», поэтому `sellsRequestedFamily: undefined` ничего не меняет в вердикте.
- `лимит: N л` в строке станции — минимальный известный лимит на АИ-95. Протухает только *наблюдение* лимита (`observedAt`, например `last_at` у gdebenz): давно виденный лимит мог исчезнуть, и пускать его в минимум нельзя. Лимит, про который источник говорит «действует с» (`inForceSince` у Benzonavt) или который публикуется без даты (2GIS), остаётся в силе независимо от возраста этой даты — отбросить его значило бы занизить ограничение поездки. У наблюдённого и уже не свежего лимита рядом печатается возраст: `лимит: 20 л (3 ч назад)`. Это ограничение поездки, а не признак наличия.
- `trust` у observation (`confidence`, `confirmations`, `reports`, `basis`, `reports_count`, `author_*`) — заявленная источником собственная уверенность. Это provenance для диагностики; никогда не выдавай её за уверенность Fuel Watch и не показывай пользователю как вероятность.
- Расхождение видимой карточки и скрытого поля источника — структурный конфликт, а не повод выбрать «удобное» поле. У gdebenz марка, названная только в `fuels_now` или только в тексте карточки, даёт `UNCERTAIN` с сохранением обеих формулировок в `conflict`, и такой позитив не попадает в «Куда ехать». Другой источник с собственным прямым позитивом по этой АЗС при этом остаётся в силе.
- `renderReport` возвращает отдельное поле `diagnostics` (`agentOnly: true`) с полным списком warnings, source health, runtime, identity-диагностикой и осиротевшими процессами. Это внутренние данные: не цитируй их пользователю и не выдавай за ограничение поездки. Ограничения, влияющие на поездку, уже напечатаны в каноническом Markdown человеческим языком.
...
## Разовая произвольная география

`collect.mjs --area <файл> --output <файл>` собирает одну зону для одного запуска. Настроенный многоугольник, `~/.fuel-watch/state/latest.json` и общая история при этом не трогаются, поэтому `--output` обязателен. Файл содержит объект зоны (или `{"area": {...}}`) любого поддерживаемого вида, включая `route-corridor` с явными `waypoints` и `corridorWidthMeters`, и проверяется по той же схеме, что и настроенная зона, поэтому ошибка приходит с именем поля. Зона может объявить собственные потолки `maxAreaSquareKm` и `maxStationCount`: превышение площади останавливает запуск до опроса источников, превышение числа станций — сразу после сведения идентичностей. Это не новый default: постоянная зона меняется только через `resolve-area.mjs`.
...
- `CHALLENGE`, `HTTP_ERROR`, `TIMEOUT`, `RESOURCE_BLOCKED`, `SCHEMA_CHANGED`, `NAVIGATION_FAILED`, `PAGE_LOST`: назови источник и продолжи с остальными. Отчёт уже печатает строку «Недоступные источники: …» с человеческой причиной и перечисляет, по каким источникам построена оценка; публикуй её как есть.
- Все источники деградировали: сообщи «нет доступных свежих данных», не «бензина нет».
- `CHALLENGE` и user-in-the-loop: skill никогда не решает и не обходит CAPTCHA. Если в конфиге включён `browser.challengeHandover` (по умолчанию выключен и работает только в headed-режиме), collect удерживает уже открытое окно ограниченное время, чтобы человек решил проверку сам, только перечитывает страницу и повторяет источник один раз. Ни кликов, ни ввода, ни отправки форм со стороны skill не происходит. Исход записан в `runtime.challengeHandovers`: `CLEARED` — страница проверки исчезла (кто именно её убрал, мы не наблюдаем, и утверждать этого нельзя), `TIMED_OUT` — не исчезла за отведённое время, `NOT_OBSERVABLE` — адаптер распознал проверку по данным страницы, а видимого маркера нет, поэтому окно не удерживалось, `UNREADABLE` — страница не читалась уже при первой пробе, поэтому окно не удерживалось, `LOST_WHILE_HELD` — окно удерживалось, но страница перестала читаться во время ожидания. Лимит `maxPerRun` расходуют все исходы, при которых окно действительно удерживалось: `CLEARED`, `TIMED_OUT` и `LOST_WHILE_HELD`.
- `HTTP_429_LIMITED` у Yandex: источник отдал страницу автолимита `limited`. Collect уже делает одну повторную попытку с той же личностью браузера без токена `Headless`; если код всё равно дошёл до отчёта, это настоящий лимит, а не отсутствие бензина. Не обходи его новыми попытками и не подменяй stealth-флагами.
- Осиротевшие браузеры: чистый `close` удаляет pid-файл namespace, поэтому живой agent-browser процесс, всё ещё записанный под созданным этим прогоном namespace, — сирота. Такие процессы попадают в `runtime.orphanProcesses` и по умолчанию завершаются (`SIGTERM`, затем `SIGKILL`) в пределах того же бюджета очистки; `browser.reapOrphanProcesses: false` оставляет только запись. Сигнал отправляется только если совпали три условия: pid записан под созданным этим прогоном namespace, это тот же pid, который мы видели обслуживающим namespace во время прогона, и исполняемый файл процесса действительно называется `agent-browser*`. Общего сканирования процессов нет: оно задело бы браузеры других сессий на той же машине.
