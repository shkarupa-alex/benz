...
5. Удали временный каталог. Компактная история статусов и часовых rolling-count сигналов по бензиновым маркам 92/95/98/100 сохраняется отдельно в пользовательском state-каталоге, автоматически обрезается до последних 7 дней и используется следующими разовыми проверками и monitoring tick’ами. Дизель, газ и неизвестные продукты исключаются. Путь можно явно задать через `--history`; переменная `FUEL_WATCH_HISTORY_PATH` меняет default path.
