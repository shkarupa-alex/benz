...
Штатная collection всегда использует `HEADED` Chromium без stealth-флагов и подмены fingerprint. Если доступен пользовательский display, BrowserRunner сохраняет `DISPLAY`/`WAYLAND_DISPLAY`/`XAUTHORITY`; иначе `agent-browser` может использовать собственный Xvfb, но браузер всё равно остаётся non-headless. Не переноси результат `HEADLESS`-диагностики на обычный браузер или доступность сайта вообще.

Если пользователь просит одновременно показать все источники, открой **одно окно с тремя вкладками**: используй один namespace и одну session, первую страницу открой через `open --headed`, остальные — через `tab new --label yandex|gdebenz|2gis`. Не создавай отдельную session для каждого источника: отдельные sessions запускают отдельные окна. Видимая диагностическая session не является collection-run и закрывается только по команде пользователя либо после согласованного осмотра.
