...
`collect.mjs --area <файл> --output <файл>` собирает одну зону для одного запуска. Настроенный многоугольник, `~/.fuel-watch/state/latest.json` и общая история при этом не трогаются, поэтому `--output` обязателен. Файл содержит объект зоны (или `{"area": {...}}`) любого поддерживаемого вида, включая `route-corridor` с явными `waypoints` и `corridorWidthMeters`. Зона может объявить собственные потолки `maxAreaSquareKm` и `maxStationCount`: превышение площади останавливает запуск до опроса источников, превышение числа станций — сразу после сведения идентичностей. Это не новый default: постоянная зона меняется только через `resolve-area.mjs`.
...
- `CHALLENGE` и user-in-the-loop: skill никогда не решает и не обходит CAPTCHA. Если в конфиге включён `browser.challengeHandover` (по умолчанию выключен и работает только в headed-режиме), collect удерживает уже открытое окно ограниченное время, чтобы человек решил проверку сам, только перечитывает страницу и повторяет источник один раз. Каждая такая передача записана в `runtime.challengeHandovers` с исходом `SOLVED_BY_USER` или `TIMED_OUT`. Ни кликов, ни ввода, ни отправки форм со стороны skill не происходит.
