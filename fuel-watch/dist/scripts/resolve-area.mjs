...
import {
  BrowserRunner
} from "./chunks/chunk-AEBOKPWX.mjs";
import {
  ensureUserConfig
...
async function verifyAnchors(configPath, browserFactory = (config) => new BrowserRunner(config)) {
  ...
  try {
    for (const anchor of config.area.anchors) {
      ...
      await browser.waitReady({ anyOfSelectors: ["script", "[data-chunk=search-result]", ".search-snippet-view"], urlRejectPatterns: ["captcha", "showcaptcha"], timeoutMs: 2e4 });
...
function approximateMeters(a, b) {
  const dy = (a[1] - b[1]) * 111320, dx = (a[0] - b[0]) * 111320 * Math.cos(a[1] * Math.PI / 180);
  return Math.hypot(dx, dy);
}
async function writeResolvedAnchors(configPath, values) {
...
async function main() {
  ...
  const values = await verifyAnchors(args.config);
  process.stdout.write(`${stableJson(values)}
`);
  if (args.write) {
    if (!args.confirm) throw new Error("--write requires --confirm");
    await writeResolvedAnchors(args.config, values);
  }
}
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    const arg = argv[i];
    if (arg === "--config") out.config = resolve(argv[++i]);
