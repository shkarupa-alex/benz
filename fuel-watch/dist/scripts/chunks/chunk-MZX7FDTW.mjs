...
var BrowserRunner = class {
  ...
  async snapshot() {
    ...
    const title = await this.commandJson(["get", "title", "--json"], { timeoutMs: 1e4 });
    const text = await this.commandJson(["get", "text", "body", "--json"], { timeoutMs: 1e4 });
    for (const part of [url, title, text]) if (part.exitCode !== 0) throw classifyCommandFailure(part, "snapshot");
    const snapshot = { url: String(unwrapJson(url.json) ?? ""), title: String(unwrapJson(title.json) ?? ""), textPrefix: clampText(unwrapJson(text.json), 1e3) };
