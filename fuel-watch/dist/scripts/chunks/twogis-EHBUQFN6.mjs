...
// scripts/lib/sources/twogis.mjs
var id = "2gis";
...
async function collect(request, ctx) {
  try {
    ...
    await ctx.browser.waitReady({ anyOfSelectors: ["[class*=searchResults]", "a[href*=firm]"], urlRejectPatterns: ["/captcha", "/museum", "recaptcha"], timeoutMs: Math.min(2e4, ctx.config.browser.adapterTimeoutMs) });
...
function twogisExtractor(polygon, detailTimeoutMs = 3500, detailBudgetMs = 1e4) {
...
var TWOGIS_EXTRACTOR = twogisExtractor();
export {
  TWOGIS_EXTRACTOR,
  capability,
  collect,
