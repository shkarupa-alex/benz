...
function requireStateDir(args) { if (!args["state-dir"]) throw new Error("--state-dir is required"); return requireSafeLocation(resolve(args["state-dir"]), monitorRoot()); }
function requireSafeLocation(stateDir, root) { const rel = relative(root, stateDir); if (!rel || rel.startsWith("..") || isAbsolute(rel)) throw new Error(`State directory must be a child of ${root}`); return stateDir; }
async function assertRealLocation(stateDir) { const root = monitorRoot(); const info = await lstat(stateDir); if (!info.isDirectory() || info.isSymbolicLink()) throw new Error("State directory must be a real directory, not a symlink"); const [rootPath, statePath] = await Promise.all([realpath(root), realpath(stateDir)]); const rel = relative(rootPath, statePath); if (!rel || rel.startsWith("..") || isAbsolute(rel)) throw new Error("State directory resolves outside the Fuel Watch monitor root"); }
...
async function main() { const [command, ...argv] = process.argv.slice(2); const args = parseArgs(argv); if (command === "init" && !args.config) args.config = await ensureUserConfig({ templateConfigPath: defaultConfigPath, templateBrowserConfigPath: defaultBrowserConfigPath, templateSchemaPath: defaultSchemaPath }); process.stdout.write(`${stableJson(await monitorCommand(command, args))}\n`); }
