...
import { stableJson } from "./lib/util.mjs";
...
async function main() { const args = parseArgs(process.argv.slice(2)); const values = await verifyAnchors(args.config); process.stdout.write(`${stableJson(values)}\n`); if (args.write) { if (!args.confirm) throw new Error("--write requires --confirm"); const config = await loadConfig(args.config); config.area.anchors = values.map(({ resolvedPoint, driftMeters, ...anchor }) => ({ ...anchor, point: resolvedPoint ?? anchor.point })); await writeFile(resolve(args.config), `${JSON.stringify(config, null, 2)}\n`); } }
