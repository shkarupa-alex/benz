...
export function normalizeCoordinate(value, order = "LON_LAT") { if (!Array.isArray(value) || value.length < 2) return [NaN, NaN]; const a = Number(value[0]), b = Number(value[1]); return order === "LAT_LON" ? [b, a] : [a, b]; }
...
export function normalizeStatus(value) {
  const text = String(value ?? "").normalize("NFKC").toLowerCase().replaceAll("ё", "е").replace(/\s+/g, " ").trim();
  if (!text || /^(?:нет данных|неизвестно|unknown|no data|n\/a)$/u.test(text)) return "UNKNOWN";
  if (/\b(?:unavailable|not\s+available|out[\s-]*of[\s-]*stock|sold\s+out)\b/u.test(text) || /(?:^|\s)(?:нет|не)\s+(?:в\s+)?налич(?:ии|ие)(?:\s|$)/u.test(text) || /(?:^|\s)отсутств\p{L}*(?:\s+в\s+наличии)?(?:\s|$)/u.test(text) || /^нет$/u.test(text)) return "OUT_OF_STOCK";
  if (/\b(?:limited|low\s+stock)\b/u.test(text) || /(?:^|\s)(?:мало|заканч\p{L}*)(?:\s|$)/u.test(text)) return "LIMITED";
  if (/\b(?:in[\s-]*stock|available)\b/u.test(text) || /(?:^|\s)(?:есть|в\s+наличии)(?:\s|$)/u.test(text)) return "IN_STOCK";
