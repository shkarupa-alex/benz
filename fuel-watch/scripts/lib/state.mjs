export function updateAvailabilityRuns(previousRuns = [], assessments, previousSnapshot, fetchedAt) {
  ...
  for (const assessment of assessments) {
    ...
    if (currentPositive) {
      if (old?.state === "AVAILABLE") { old.lastConfirmedAt = fetchedAt; old.negativeTicks = 0; runs.set(key, old); }
      else runs.set(key, { stationKey: assessment.stationKey, productKey, state: "AVAILABLE", firstObservedAt: fetchedAt, lastConfirmedAt: fetchedAt, transitionWindow: oldVerdict === "NOT_AVAILABLE" ? { after: previousSnapshot.fetchedAt, atOrBefore: fetchedAt } : undefined, basis: oldVerdict === "NOT_AVAILABLE" ? "OBSERVED_TRANSITION" : "FIRST_SEEN", negativeTicks: 0 });
